...
import { Loader2, FileText, Users, Clock, CheckCircle, Activity, Menu, X, Home, FileSignature, Settings, LogOut, ChevronDown, ChevronRight, Search, Plus, FileScanIcon, ScanIcon, Mic, MicOff, Scale } from 'lucide-react';
...
export default function DocumentGenerator() {
  ...
  useEffect(() => {
    ...
    recognitionInstance.lang = language === 'mr' ? 'mr-IN' : language === 'hi' ? 'hi-IN' : language === 'gu' ? 'gu-IN' : 'en-IN';
    ...
    recognitionInstance.onresult = (event: any) => {
      ...
      setFormData((prev) => ({
        ...prev,
        additionalDetails: `${prev.additionalDetails} ${transcript}`.trim(),
      }));
    };
  ...
  const generateDocument = async () => {
    ...
    try {
      ...
      if (user?.uid && documentType) {
        ...
        const docId = await saveDocumentGeneration(
          user.uid,
          documentType,
          title,
          language || 'en',
      ...
      const message = error instanceof Error ? error.message : 'Error generating document. Please try again.';
      setDocumentContent(`Error generating document: ${message}`);
    } finally {
          ...
          >
            ...
            <div className="flex-1 overflow-y-auto py-4">
              <nav className="px-4 space-y-1">
                <a
                  href="/dashboard"
                  className={`flex items-center px-4 py-3 text-sm font-medium rounded-lg ${pathname === '/dashboard'
                ...
                <a
                  ...
                  className={`flex items-center px-4 py-3 text-sm font-medium rounded-lg ${pathname === '/document-generator'
                      ? 'bg-blue-50 text-blue-700'
                      : 'text-gray-700 hover:bg-blue-50 hover:text-blue-600'
                    }`}
                >
                ...
                <a
                  ...
                  className={`flex items-center px-4 py-3 text-sm font-medium rounded-lg ${pathname === '/document-scanner'
                      ? 'bg-blue-50 text-blue-700'
                      : 'text-gray-700 hover:bg-blue-50 hover:text-blue-600'
                    }`}
                >
                ...
                <a
                  ...
                  className={`flex items-center px-4 py-3 text-sm font-medium rounded-lg ${pathname === '/notary-consulting'
                      ? 'bg-blue-50 text-blue-700'
                      : 'text-gray-700 hover:bg-blue-50 hover:text-blue-600'
                    }`}
                >
                ...
                <div className="space-y-4">
                  ...
                  <div>
                    <div className="flex items-center justify-between mb-1">
                      ...
                      <div className="flex items-center gap-2">
                        <button
                          ...
                          disabled={!formData.additionalDetails}
                    ...
                    <p className="mt-1 text-xs text-gray-500">
                      {voiceSupported ? 'Mic se bolkar details add kar sakte ho.' : 'Voice input is not supported in this browser.'}
                    </p>
                  </div>
                  ...
                  <button
                    ...
                    className={`w-full py-2 px-4 rounded-md text-white font-medium ${!documentType || isGenerating
            ...
            <div className="lg:col-span-2">
              <div className="bg-white p-6 rounded-lg shadow-md h-full">
                ...
                <div className="mt-4 flex justify-end space-x-2">
                  <button
                    onClick={async () => {
                      ...
                      try {
                        ...
                        const htmlBlob = new Blob([documentContent], { type: 'text/html' });
                        ...
                        reader.onloadend = async () => {
                          ...
                          if (data.url && currentDocId) {
                            const { updateDocumentWithPdfUrl } = await import('@/lib/firestore');
                            await updateDocumentWithPdfUrl(currentDocId, data.url);
                          }
                  ...
                  >
                    {isSavingToCloud ? 'Saving...' : 'Save Draft'}
                  </button>
                  <button
                    onClick={async () => {
                      if (!documentContent) {
                    ...
                    `;
                        ...
                        // Automatically upload to Cloudinary for the dashboard
                        if (currentDocId) {
                          try {
                            setIsSavingToCloud(true);
                            // Get base64 string
                            const pdfBaseUrl = await worker.outputPdf('datauristring');
                            ...
                            console.error('Failed to upload PDF to cloud:', uploadError);
                          } finally {
                            setIsSavingToCloud(false);
                          }
                        }
                  ...
                  >
                    {isSavingToCloud ? 'Saving to Cloud...' : 'Download as PDF'}
                  </button>
                </div>
              </div>
            </div>
