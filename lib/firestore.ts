...
export interface DocumentRecord {
  ...
  cloudinaryUrl?: string; // PDF URL on Cloudinary
  createdAt: Timestamp;
  metadata?: {
    partyName?: string;
...
export async function updateDocumentWithPdfUrl(
  ...
  try {
    ...
    console.log(`Document ${docId} updated with Cloudinary URL`);
  } catch (error) {
    console.error('Error updating document with PDF URL:', error);
  }
}
